...
여기서 방향 벡터의 크기는 1일 수도, 그렇지 않을 수도 있다. 우리는 앞으로 방향 벡터를 1로 정규화해준 뒤 그것에 실수(t)를 곱하여 광선의 방향을  것이다.\
` P(t) = O + t * D ` \
`(D는 정규화된 방향벡터)`\
...
***이미지3: 광선이 우리 눈(=카메라)에서 나간다고 생각하자***
...
***이미지4: 뷰포트와 픽셀. 큰 사각형이 뷰포트, 작은 사각형이 픽셀이다.***
...
```
***Code6 : [/src/scene/canvas.c]***

뷰포트는 아래 그림(**이미지6**)과 같이 설정해줄 것이다.
![](./images/image4.jpeg)
***이미지6: 제대로 그려서 수정하겠듬***
